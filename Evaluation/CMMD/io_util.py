...
class CMMDDataset(Dataset):
    def __init__(self, path, reshape_to, max_count=-1):
        ...
        print("Reshape to:", self.reshape_to)
        self.max_count = max_count
        img_path_list = self._get_image_list()
        if max_count > 0:
            img_path_list = img_path_list[:max_count]
        self.img_path_list = img_path_list
        print("Total num. of images:", len(self.img_path_list), "with max_count:", max_count)
...
def compute_embeddings_for_dir(
    ...
    """
    print("Presaved path:", presaved_feat_path)
    if  os.path.exists(presaved_feat_path):
        
        random.seed(seed)
        
        print("Load existing features")
        all_embs = np.load(presaved_feat_path).tolist()
        #feats_real = np.float32(feats_real)
        print("Total num. of image features:", len(all_embs))
        #exit()
        if sample_it and  len(all_embs) > max_number_of_samples: 
            all_embs = random.sample(all_embs, max_number_of_samples)
            print("Samples num. of images:", len(all_embs))
    ...
    np.save(presaved_feat_path, arr=all_embs)
