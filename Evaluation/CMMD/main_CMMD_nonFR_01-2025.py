...
def compute_cmmd(ref_dir, eval_dir, feature_save_folder,  max_number_of_samples, sample_it, seed, ref_embed_file=None, batch_size=32, max_count=-1):
    ...
    if ref_embed_file is not None:
        ...
        ref_embs = io_util.compute_embeddings_for_dir(ref_dir, embedding_model, batch_size, presaved_real_feat_path, 
                                                      max_number_of_samples, sample_it = sample_it, seed = seed, max_count= max_count, non_FR_dataset=True).astype("float32")
...
def main(argv):
    feature_save_folder = "CMMD_NonFR_Features_12-2024"
    if len(argv) != 3:
    ...
    output_folder = f"CMMD_NonFR_Results_12-2024_Seed{seed}_Sampled-{sample_it}_{max_number_of_samples}/{dir1.split('/')[-2]}_vs_{dir2.split('/')[-2]}"
