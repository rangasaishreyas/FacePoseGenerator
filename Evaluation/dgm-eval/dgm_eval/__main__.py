...
def compute_scores(args, reps, test_reps, labels=None):
    ...
    if 'vendi' in args.metrics:
        ...
        scores['std vendi per class'] = vendi_scores.std()
...
def save_score(scores, output_dir, model, path, ckpt, nsample, is_only=False):
    ...
    if is_only:
        ...
        #out_str = f"{model}_{'-'.join([os.path.basename(p) for p in path])}{ckpt_str}_nimage-{nsample}_FINAL.txt"
        out_str = f"{model}_{'_vs_'.join([os.path.basename(p) for p in path])}{ckpt_str}-{nsample}.json"
