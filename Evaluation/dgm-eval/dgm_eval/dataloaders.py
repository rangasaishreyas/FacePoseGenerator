...
class DataLoader():
    ...
    def __init__(self, path, train_set=False, nsample=-1, transform=None,
        ...
        self.get_dataset()
        print("Total samples: ", len(self.data_set))
        if (self.nsample > 0) and (len(self.data_set) > self.nsample + 2000):
            print("Total dataset is 2000 larger than", self.nsample)
            print("Subsample it to", self.nsample)
