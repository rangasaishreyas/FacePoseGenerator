...
@torch.no_grad()
def extract_features(datadir, batchsize, num_imgs, fr_path, num_ids=0, device="cuda:0"):
    ...
    encoder =  iresnet100(num_features=512, dropout=0.0).to(device)
...
############################################
@torch.no_grad()
def extract_features_combined_images(datadir, batchsize, num_imgs, fr_path, num_ids=0, device="cuda:0"):
    """feature extraction of images in datadir with FR model specified in fr_path
    args:
        datadir: path to dir containing images
        batchsize: batch size
    ...
    embs = []
    img_paths = []
    desired_img_size = 512 # TODO +
    #img_padding = 10 
    
    print("REMOVE THE QUICKFIX for solving padding issues")
    do_padding_quickfix = False 

    for images, img_path in tqdm(loader):
        images_of_ID = []
        if do_padding_quickfix: images = images[:, :, 10:, 10:]
        _, c, h, w = images.shape
        
        splits_h = int(h / (desired_img_size )) # TODO remove quickfix 
        splits_w = int(w / (desired_img_size )) 
        
        sub_imgs = torch.chunk(images, splits_h, dim=2)

        for sub_img in sub_imgs:     
            if do_padding_quickfix: sub_img = sub_img[:, :, :-10, :]
            sub_sub_imgs = torch.chunk(sub_img, splits_w, dim=3)
            
            for sub_sub_img in sub_sub_imgs:
                if do_padding_quickfix: sub_sub_img = sub_sub_img[:, :, :, :-10]
                images_of_ID.append(sub_sub_img)

        
        images_of_ID = torch.cat(images_of_ID)
        images_of_ID = images_of_ID.to(device)
        
        # Already normalized ...
        images_of_ID = transforms.functional.resize(images_of_ID, 112)
        #images_of_ID = transforms.functional.normalize(images_of_ID, mean=[0.5], std=[0.5])
        
        emb = encoder(images_of_ID)
        emb = F.normalize(emb)
        emb = emb.detach().cpu().numpy()
        # print(emb.shape)
        # exit()
