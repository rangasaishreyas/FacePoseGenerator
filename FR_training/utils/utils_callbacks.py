...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            ...
            self.logger.info(
                "[%s][%d] XNorm: %f, Accuracy-Flip: %1.5f+-%1.5f, Accuracy-Highest: %1.5f" % (self.ver_name_list[i], global_step, xnorm, acc2, std2,  self.highest_acc_list[i])
            )
...
class CallBackLogging(object):
    def __init__(self, frequent, rank, total_step, batch_size, world_size, logger):
        self.frequent: int = frequent
        self.rank: int = rank
    ...
    def __call__(
        ...
        #print(self.rank, global_step, self.frequent)
        if self.rank == 0 and global_step > 0 and global_step % self.frequent == 0:
            if self.init:
                ...
                self.logger.info("Initialize logging")
                self.init = True
                self.tic = time.time()
...
class CallBackModelCheckpointOld(object):
    ...
    def __call__(
        ...
        #         torch.save(
        #             backbone.state_dict(),
        #             os.path.join(self.output, str(global_step) + "_backbone.pt"),
        #         )
       # else:
        #if saving_best or global_step > 100 and self.rank == 0:
        torch.save(
